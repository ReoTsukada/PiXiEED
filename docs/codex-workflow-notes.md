...
- 広場のPC/スマホ表示、非スクロール、AdSenseフッター、上下左右ボタン削除、ズーム、タップ移動、自キャラ中央固定、歩行表示、吹き出しコメントは次で確認する。
  - `node --check contest/assets/js/plaza.js`
  - `node scripts/verify-plaza.mjs`
- `scripts/verify-plaza.mjs` は空きポートで `scripts/static-server.mjs` を起動し、`http://127.0.0.1:<port>/contest/` をChrome/Playwrightで検証する。手動で8001/8002を探す前にこのスクリプトを使う。
...
- `contest/index.html` はユーザーが `file://` で直接開く場合がある。`plaza.js` は `type="module"` にしない。Chromeではfile直開きのmodule scriptがCORSでブロックされ、キャラ生成・カメラ・ボタンがすべて止まる。
- `scripts/verify-plaza.mjs` は `http://127.0.0.1:<port>/contest/` に加えて `file://.../contest/index.html` のスモークチェックも行う。file直開きで自キャラ、移動、吹き出しが動くことを維持する。
