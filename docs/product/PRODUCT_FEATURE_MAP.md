...
| GAME | GameProject、Scene、Entity、Component、Prefab、Behavior IR、Input、Runtime、Build、Preview | `pixiedraw2/src/game/game-300`〜`game-350` のCore、`/igame/?product=...` 公開Player、Market Bootstrap／PXD Canvas Runtime。Core ShellのGame Studio routeはComing Later | `GAME-300`〜`GAME-350` | `PARTIAL` | 公開Playerのコード経路は接続済み。Game Studio route、Supabase適用、外部Build／実機資格化は別境界で未受入れ | `igame/index.html`, `pixiedraw2/src/game/`, `supabase/functions/igame-player-bootstrap/`, `core-shell/assets/routes/game-route.js` |
