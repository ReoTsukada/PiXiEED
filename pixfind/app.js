...
const LEGACY_CONTEST_BUCKET = 'pixieed-contest';
...
function isContestPublicAssetUrl(value) {
  ...
  return normalized === LEGACY_CONTEST_PUBLIC_BASE || normalized.startsWith(`${LEGACY_CONTEST_PUBLIC_BASE}/`);
}
...
function isLegacyContestPuzzleAssetUrl(value) {
  ...
  try {
    return new URL(normalized).pathname.includes(`/${LEGACY_CONTEST_BUCKET}/puzzles/`);
  } catch (_) {
