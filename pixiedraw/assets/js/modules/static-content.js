(() => {
  ...
  const HELP_GUIDE_ITEMS = Object.freeze([
    ...
    Object.freeze({
      id: 'palette-mode',
      title: { ja: 'パレット', en: 'Palette', zh: '调色板' },
      keywords: { ja: 'インデックス パレット 色追加 色編集', en: 'indexed palette add color edit', zh: '索引 调色板 添加颜色 编辑颜色' },
      points: {
        ja: [
          'PiXiEEDrawは最大256色のインデックスカラーで描画します。',
          '「+」で色追加、「-」で削除ができます。',
          '貼り付け時は、既存パレットの近似色へ合わせます。',
        ],
        en: [
          'PiXiEEDraw draws with indexed color, up to 256 colors.',
          'Use + to add colors and - to remove them.',
          'Pasted colors snap to the nearest existing palette color.',
        ],
        zh: [
          'PiXiEEDraw使用最多256色的索引颜色绘制。',
          '可用“+”添加颜色，用“-”删除颜色。',
          '粘贴时会贴近现有调色板的近似色。',
        ],
      },
    ...
    Object.freeze({
      ...
      points: {
        ja: [
          ...
          '開始後に「参加コードをコピー」を押して、相手へコードを共有します。',
          '参加する人は、プロジェクト一覧またはシェアモードへ参加コードを入力します。未ログインの場合も参加待ちカードが作成され、ログイン後に参加を再開できます。',
          '描画とコメントはリアルタイムで共有され、シェアプロジェクトのカードからあとで開き直せます。',
          '無料の作成枠は1アカウントにつき1枠です。「作成枠」から1枠100円で必要な数をまとめて買い切り追加できます。',
          '他の人のシェアプロジェクトへの参加は作成枠を消費しません。購入後はStripeから戻ると枠が自動更新されます。',
          '上限に達した場合は、共有中のプロジェクトを選んで開き、内容を確認してローカル専用へ戻すか、追加枠を購入できます。',
          '共有解除後のサーバーデータは、参加者全員のローカル保存が確認できてから削除されます。',
        ],
        en: [
          'After signing in, open Share Mode, select Start Share Mode, and confirm the dialog.',
          'Use Copy Join Code to invite someone.',
          'Participants enter the join code from the project list or Share Mode. A pending card is kept until sign-in is complete.',
          'Drawing and comments are shared in real time, and the shared-project card can be reopened later.',
          'Each account includes one creation slot. Additional permanent slots are available as a one-time purchase for JPY 100 each.',
          'Joining a project owned by someone else does not use a creation slot. The slot count refreshes after returning from Stripe.',
          'At the limit, choose an existing shared project to reopen and convert to local-only, or purchase another slot.',
          'Server data is removed only after every participant confirms a successful local save.',
        ],
        zh: [
          '登录后打开共享模式，按“开始共享模式”，并在确认对话框中开始。',
          '开始后可通过“复制参加码”邀请对方。',
          '参与者可在项目列表或共享模式中输入参加码。未登录时会保留待参加卡片，登录后继续参加。',
          '绘制和评论会实时共享，也可稍后从共享项目卡片重新打开。',
          '每个账号免费包含1个创建名额。可通过一次性支付100日元永久增加1个名额。',
          '参加他人创建的共享项目不会占用创建名额。从Stripe返回后名额会自动更新。',
          '达到上限时，可选择并打开现有共享项目，将其改为仅本地项目，或购买追加名额。',
          '只有确认所有参与者都已成功保存到本地后，服务器数据才会被删除。',
        ],
      },
    }),
    Object.freeze({
  ...
  const BUILTIN_UPDATE_HISTORY_ENTRIES = Object.freeze([
    Object.freeze({
      ...
      details: Object.freeze([
        ...
        'シェアモードから共有を開始し、参加コードをコピーして相手を招待できます。',
        '受け取った人は、プロジェクト一覧またはシェアモードへ参加コードを入力して参加できます。未ログインの場合も参加待ちカードが保存され、ログイン後に参加を再開できます。',
        '描画とコメントをリアルタイムで共有し、シェアプロジェクトのカードからあとで開き直せます。',
        '無料の作成枠は1アカウントにつき1枠です。1枠100円の買い切りで、購入画面から必要な数（最大20枠）をまとめて追加できます。',
        '購入はStripe Checkoutで行い、決済完了をサーバーで確認してから作成枠へ反映します。他の人のプロジェクトへの参加は枠を消費しません。',
        '無料枠は既存の共有件数にかかわらず常に1枠です。既存の超過プロジェクトは残し、超過件数を整理対象として表示します。',
        '上限到達時は共有中のプロジェクトを選んでローカル専用へ戻すか、1枠100円で追加できます。共有データは参加者全員のローカル保存確認後にサーバーから削除します。',
      ]),
    }),
    Object.freeze({
      id: '2026-07-24-drawing-performance-preview',
      at: '2026-07-24T18:30:00+09:00',
      title: '描画・プレビューの動作を軽量化',
      published: true,
      details: Object.freeze([
        '塗りつぶしを高速化し、大きな同色領域でも処理負荷を軽減。透明部分を含む同色塗りつぶしの判定も改善。',
        '大きなブラシ・図形・貼り付け時の一括処理を最適化し、Undo履歴は変更範囲を中心に保持するよう改善。',
        'キャンバス全体が見えるズームアウト時の描画とレイヤー・フレーム再合成を軽量化。',
        '操作中の描画プレビューを軽くし、確定時にまとめて適用する処理を改善。',
        '小窓プレビューを低解像度表示へ変更し、大きなキャンバスでも負荷を抑制。参照表示中の不要な更新も停止。',
      ]),
    }),
    Object.freeze({
      id: '2026-07-21-v2-runtime-icons-actions',
      at: '2026-07-21T17:30:00+09:00',
      title: 'V2軽量化と再生・出力・操作UIを更新',
      published: true,
      details: Object.freeze([
        '新規プロジェクトのピクセルデータをUint8Array基準へ統一し、透明色を0として扱う軽量な編集形式へ更新。',
        'フレーム・レイヤー追加と端末内V2自動保存を差分中心にし、大規模アニメーションでの待ち時間とメモリ負荷を軽減。',
        '再生中に保存向け圧縮やキャッシュ破棄を繰り返さないよう修正し、保存済み再生状態からの復帰にも対応。',
        'PNG・GIF・SpriteMAP・PXDで共通する合成処理を修正し、空の画像が出力される問題を解消。',
        'RGBカラーの選択範囲で、移動・カット・コピー・貼り付け時に実色を透明として扱ってしまう問題を修正。',
        'V2自動保存で、RGB直色情報を持つ空レイヤーが復元できなくなる問題を修正し、既存の不整合データも読み込めるよう対応。',
        '形式マーカーが欠けたUint8レイヤーデータを互換復元し、保存済みプロジェクトを開けない場合がある問題を修正。',
        '直色情報を持つレイヤーで補助インデックス配列が破損・欠落していても、描画を維持して復元できるよう改善。',
        '選択移動で、直色情報の移動元が描画キャッシュに残り、コピーされたように見える問題を修正。',
        '旧形式を含む直色情報の選択移動で、透明値の変換により移動先の色が消える問題を修正。',
        '選択移動やキャンセル中のUndoで、保留中の差分履歴を戻せず停止する問題を修正。',
        '矩形・投げ縄・同色を含む選択操作で、選択範囲の内側（透明部分や凹みを含む）を押しても確定・解除しないよう改善。',
        'ペン、消しゴム、スポイト、塗り、選択、対称、仮想カーソル、レイヤー、ショートカット一覧のアイコンを更新。',
        'ホームでも共通アクションバーを常に初期化し、詳細メニューが表示されない場合がある問題を修正。',
      ]),
    }),
    Object.freeze({
  ...
  const UI_LANGUAGE_ZH_MAP = Object.freeze({
    ...
    '自動参加': '自动加入',
    '自動参加: 参加コードは参加者入室になります。': '自动加入：参加码会以参与者身份进入。',
    '色相': '色相',
    '色相順でソート': '按色相排序',
    '表示': '显示',
    '表示PX': '显示PX',
