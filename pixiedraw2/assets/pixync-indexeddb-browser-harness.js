...
async function validatePixyncDraft(draft2) {
  if (
    !isRecord(draft2) || draft2.schemaVersion !== PIXYNC_DRAW2_SCHEMA_VERSION
  ) {
    reject(
      "INVALID_ENVELOPE",
...
var PixyncDurableJournal = class _PixyncDurableJournal {
  ...
  static async open(projectId, persistence, options = {}) {
    ...
    const journal = new _PixyncDurableJournal(
      projectId,
      persistence,
      options,
      state,
  ...
  async enqueue(draft2) {
    await validatePixyncDraft(draft2);
...
var PIXYNC_INDEXEDDB_PERSISTENCE_SCHEMA =
  "PIXYNC_DRAW2_INDEXEDDB_SNAPSHOT_V1";
...
// assets/pixync-indexeddb-browser-harness.ts
var HARNESS_VERSION = "PIXYNC-DRAW2-140-BROWSER-HARNESS-V1";
var DEFAULT_DB_NAME = "pixync-draw2-140-browser-harness-v1";
...
async function runProjectChecks(dbName, prepared) {
  const checks = [];
  const projectA = "pixync-draw2-140-project-a";
  const projectB = "pixync-draw2-140-project-b";
  const persistenceA = createPixyncIndexedDbPersistence(projectA, {
    dbName,
  });
  const persistenceB = createPixyncIndexedDbPersistence(projectB, {
    dbName,
  });
  const journalA = await PixyncDurableJournal.open(projectA, persistenceA);
  const journalB = await PixyncDurableJournal.open(projectB, persistenceB);
  if (!prepared) {
  ...
  const reopenedA = await PixyncDurableJournal.open(
    projectA,
    createPixyncIndexedDbPersistence(projectA, {
      dbName,
    }),
  );
...
async function runSaveFailureCheck(dbName) {
  const projectId = "pixync-draw2-140-save-failure";
  const source = await PixyncDurableJournal.open(
    projectId,
  ...
  try {
    await createPixyncIndexedDbPersistence(projectId, {
...
async function runTamperCheck(dbName) {
  const projectId = "pixync-draw2-140-tamper";
  ...
  let rejected = false;
  try {
    await PixyncDurableJournal.open(
      projectId,
      createPixyncIndexedDbPersistence(projectId, {
        dbName: `${dbName}-tamper`,
      }),
...
async function runOrderingCheck(dbName) {
  const projectId = "pixync-draw2-140-ordering";
  ...
  const concurrent = createPixyncIndexedDbPersistence(projectId, {
    dbName: name,
  });
  await Promise.all([
  ...
  const restored = await PixyncDurableJournal.open(
    projectId,
    createPixyncIndexedDbPersistence(projectId, {
      dbName: name,
    }),
...
async function runPixyncIndexedDbBrowserHarness(
...
export { runPixyncIndexedDbBrowserHarness };
