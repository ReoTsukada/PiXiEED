...
var AUDIO_CHIP_MACHINE_IDS = [
  "NONE",
  "GAME_BOY",
  "FAMICOM"
];
function isAudioChipMachineId(value) {
  return typeof value === "string" && AUDIO_CHIP_MACHINE_IDS.includes(value);
}
...
function validateProjectShape(project) {
  ...
  if (value.chipMachineId !== void 0 && !isAudioChipMachineId(value.chipMachineId)) {
    return fail2("AUDIO_INVALID_PROJECT", "Chip machine identifier is not supported.", "project.chipMachineId");
  }
...
async function applyAudioCommand(project, command) {
  ...
  switch (command.type) {
    ...
    case "CHIP_MACHINE_SET": {
      const chipMachineId = entityFromPayload(payload, "chipMachineId");
      if (chipMachineId === null || !isAudioChipMachineId(chipMachineId)) {
        return fail2("AUDIO_INVALID_PROJECT", "Chip machine identifier is not supported.", "command.payload.chipMachineId");
      }
      next = {
        ...next,
        chipMachineId
...
async function journalWorkspaceChipMachineSet(session, chipMachineId, mutation) {
  if (!isAudioChipMachineId(chipMachineId)) {
    return fail4("AUDIO_INVALID_PROJECT", "Workspace chip machine is not supported.", "chipMachineId");
  }
  return dispatchWorkspaceCommand(session, mutation, "CHIP_MACHINE_SET", {
    chipMachineId
  });
}
