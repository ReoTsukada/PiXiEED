...
function getChipSynthVoice(voiceId, pitchMidi, presetId = "pulse-25", overrides) {
  const normalized = String(voiceId ?? "").trim().toUpperCase().replace(/^INSTRUMENT:/u, "");
  const base = normalized === "DRUMS" ? drumVoiceForPitch(pitchMidi ?? 38, presetId) : normalized === "CHIP" || normalized.length === 0 ? voiceFromPreset(presetId) : CHIP_SYNTH_VOICE_PROFILES[normalized] ?? voiceFromPreset(presetId);
...
function voiceWaveSample(waveform, phase, seed, dutyCycle) {
...
function noteWaveSample(note, track, project, timeSeconds, sampleIndex, voiceOverrides) {
  ...
  const voice = getChipSynthVoice(renderVoiceIdForTrack(track), note.pitchMidi, renderDrumPreset(track, note.pitchMidi, project), voiceOverrides);
  ...
  let sample = voiceWaveSample(voice.waveform, phase, sampleIndex + note.pitchMidi, voice.dutyCycle);
