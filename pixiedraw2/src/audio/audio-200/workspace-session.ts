...
/** Persist the selected hardware-style chip machine without touching notes. */
export async function journalWorkspaceChipMachineSet(
  session: AudioWorkspaceSession,
  chipMachineId: AudioChipMachineId,
  mutation: AudioWorkspaceMutation,
): Promise<Audio200Result<AudioWorkspaceSession>> {
  if (!isAudioChipMachineId(chipMachineId)) {
    return fail(
      "AUDIO_INVALID_PROJECT",
      "Workspace chip machine is not supported.",
      "chipMachineId",
    );
  }
