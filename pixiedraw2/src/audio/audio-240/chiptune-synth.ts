...
export class ChipTuneSynth {
  ...
  /** Select the hardware profile used by generic CHIP notes. */
  setChipMachineId(machineId: AudioChipMachineId): void {
    this.chipMachineId = machineId;
  }
  ...
  private createNoiseSource(
    ...
    for (let index = 0; index < data.length; index += 1) {
      if (noiseMode !== "random") {
        const tap = noiseMode === "game-boy" || noiseMode === "nes-long"
          ? 1
          : 6;
