...
function renderVoiceIdForTrack(track: AudioTrack): string | undefined {
  ...
  const aliases: readonly [string, string][] = [
    ["GB_PULSE_1", "GAME_BOY_PULSE_1"],
    ["GB_PULSE_2", "GAME_BOY_PULSE_2"],
    ["GB_WAVE", "GAME_BOY_WAVE"],
    ["GB_NOISE", "GAME_BOY_NOISE"],
    ["GB_PULSE_1", "GB_PULSE_1"],
    ["GB_PULSE_2", "GB_PULSE_2"],
    ["GB_WAVE", "GB_WAVE"],
    ["GB_NOISE", "GB_NOISE"],
    ["NES_PULSE_1", "FAMICOM_PULSE_1"],
    ["NES_PULSE_2", "FAMICOM_PULSE_2"],
    ["NES_TRIANGLE", "FAMICOM_TRIANGLE"],
    ["NES_NOISE", "FAMICOM_NOISE"],
    ["NES_PULSE_1", "NES_PULSE_1"],
    ["NES_PULSE_2", "NES_PULSE_2"],
...
function deterministicLfsrNoise(
  seed: number,
  noiseMode: Exclude<ChipSynthNoiseMode, "random">,
): number {
  let lfsr = (Math.abs(Math.trunc(seed)) | 1) & 0x7fff;
...
function voiceWaveSample(
  ...
  seed: number,
  dutyCycle: number,
  noiseMode: ChipSynthNoiseMode = "random",
): number {
  if (waveform === "noise") {
    return noiseMode === "random"
      ? deterministicNoise(seed)
      : deterministicLfsrNoise(seed, noiseMode);
  }
  if (waveform === "pulse") {
