...
export interface PaletteAppendColorsPayload {
...
export type EditorCommand =
  ...
  | CommandEnvelope<PaletteAppendColorsPayload> & {
    readonly commandType: "palette.appendColors";
  };
...
function paletteColorsToAppend(
...
function validatePayload(
  ...
  if (
    ...
    command.commandType === "palette.appendColors"
  ) {
    const colors = command.commandType === "palette.appendColor"
      ? [command.payload.color]
      : command.payload.colors;
    const colorPath = command.commandType === "palette.appendColor"
      ? "payload.color"
      : "payload.colors";
    if (!Array.isArray(colors) || colors.length < 1 || colors.length > 256) {
      diagnostics.push(
        diagnostic(
          "PALETTE_COLORS_COUNT_INVALID",
          "Palette append must contain between 1 and 256 colors.",
          colorPath,
        ),
      );
      return diagnostics;
    ...
    if (asset.palette.length + colorsToAppend.length > 256) {
      diagnostics.push(
        diagnostic(
          "PALETTE_FULL",
          "Palette cannot contain more than 256 unique entries.",
          colorPath,
        ),
      );
    }
...
export class EditorCore {
  ...
  async execute(
    ...
    const paletteAppendColors = command.commandType === "palette.appendColor"
      ? paletteColorsToAppend(asset.palette, [command.payload.color])
      : command.commandType === "palette.appendColors"
      ? paletteColorsToAppend(asset.palette, command.payload.colors)
      : [];
    ...
    const nextAsset: RasterAsset = command.commandType === "palette.setColor"
      ...
      ? {
        ...asset,
        palette: [...asset.palette, ...paletteAppendColors],
        revision: asset.revision + (paletteChanged ? 1 : 0),
      }
    ...
    const payload = command.commandType === "raster.setPixel"
      ...
      ? {
        ...command.payload,
        paletteIndex: paletteChanged
          ? nextAsset.palette.length - 1
          : asset.palette.indexOf(command.payload.color),
        duplicate: !paletteChanged,
      }
      : command.commandType === "palette.appendColors"
      ? {
        colors: paletteAppendColors,
        skippedColorCount:
          command.payload.colors.length - paletteAppendColors.length,
      }
