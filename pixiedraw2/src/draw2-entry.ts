...
function loadWorkspaceModule(): Promise<WorkspaceModule> {
  ...
  workspaceChunkUrl.searchParams.set(
    "mobile",
    workspaceMobileProjectionMarker,
  );
...
const mobileColorOutputElements = Array.from(
  document.querySelectorAll<HTMLOutputElement>("[data-draw2-color-output]"),
);
...
function syncMiniPreviewLayout(): void {
  ...
  const compactPreviewHost =
    workspaceFrameElement?.dataset.workspaceProfile === "mobile" ||
    (workspaceFrameElement?.dataset.workspaceProfile === "tablet" &&
      workspaceFrameElement.dataset.workspaceOrientation === "portrait");
...
// as a separate mode because mobile Fit can be smaller than 1.0.
let viewportZoom = 1;
...
function usesResponsiveViewportFit(): boolean {
  ...
  return profile === "mobile" ||
    profile === "tablet" && orientation === "portrait";
}
...
function applyViewportDisplaySize(): void {
  ...
  // display dimensions below are always integer CSS pixels. On a responsive
  // viewport the fit scale is included here, so mobile and desktop use the
  // same source-coordinate projection.
...
function applyResponsiveViewportResize(): void {
  ...
  if (
    !usesResponsiveViewportFit() || viewportWrapElement === null ||
    viewportCenterSourcePoint === undefined
  ) {
    applyViewportTransform();
    return;
  }
  ...
  // The source point currently at the viewport centre must survive mobile
  // chrome/panel reflow as well. A normal pan clamp would move an edge point
  // back toward the middle while the responsive fit scale is changing.
  applyViewportTransform({ clampPan: false });
}
...
function zoomViewportAtClient(
  ...
  // scale, so 1.0 always means actual 100% on both desktop and mobile.
...
// Mobile HUD buttons stay on the same viewport authority as desktop wheel and
// shortcut commands.  The HUD is presentation-only; no raster or history
// state is changed by these controls.
for (
  const button of document.querySelectorAll<HTMLButtonElement>(
    "[data-mobile-viewport-command]",
  )
) {
...
async function appendColorFromDraft(): Promise<void> {
  ...
  await present(result.result.dirtyRegions, result.result.dirtyTiles);
  updateHistoryButtons();
  setColorEditorStatus(`Index ${selectedColor} · added · 1 undo`);
  setStatus(
    `Palette index ${selectedColor} added locally · canonical color changed.`,
  );
}
...
function setMirrorPreset(mode: MirrorMode): void {
  ...
  if (viewportWrapElement !== null) {
    if (mirrorEnabled) {
      viewportWrapElement.style.setProperty(
        "padding",
        usesResponsiveViewportFit() ? "38px" : "42px",
        "important",
      );
    } else {
...
function positionMirrorLineToggles(
  ...
  const placeOnViewportEdge = (
    ...
    const centerY = clampHandleCenter(
      edge === "right"
        ? viewport.height / 2
        : viewport.height - halfHeight - inset,
      halfHeight,
      viewport.height,
    );
  ...
  // Diagonal controls are deliberately tied to the viewport perimeter, not
  // to a diagonal endpoint. This keeps them away from the canvas corners
  // even when the canvas is panned, zoomed, or resized.
...
canvas.addEventListener("pointermove", (event) => {
  ...
  if (mirrorGuideDrag?.pointerId === event.pointerId) {
    moveMirrorGuideFromClient(
      mirrorGuideDrag.axis,
      event.clientX,
      event.clientY,
    );
    if (event.cancelable) event.preventDefault();
...
viewportWrapElement?.addEventListener("pointerdown", (event) => {
  ...
  if (mirrorEnabled && handleAxis !== undefined) {
    ...
    // Do not move or cancel the native button click on pointerdown. A short
    // press is an ON/OFF click; only a movement past the threshold becomes a
    // guide drag.
    return;
  }
  if (
...
viewportWrapElement?.addEventListener("pointermove", (event) => {
  ...
  if (!mirrorGuideDrag.moved) {
    const distance = Math.hypot(
      event.clientX - mirrorGuideDrag.start.x,
      event.clientY - mirrorGuideDrag.start.y,
    );
...
const finishMirrorGuideDrag = (event: PointerEvent): void => {
  if (mirrorGuideDrag?.pointerId !== event.pointerId) return;
...
// Wheel works on desktop/trackpads; a short vertical touch swipe gives the
// same bounded step on mobile while taps retain native select behavior.
let stepSwipeControl: StepControl | undefined;
let stepSwipeStartY = 0;
...
workspaceFrameElement?.addEventListener("pointerup", (event) => {
  if (stepSwipeControl === undefined) return;
  const control = stepSwipeControl;
  stepSwipeControl = undefined;
...
function runDraw2Shortcut(command: string, event?: KeyboardEvent): void {
  ...
  switch (command) {
    ...
    case "zoom-in": {
      ...
      if (anchor !== undefined) {
        zoomViewportAtClient(
          anchor.x,
          anchor.y,
          stepPixelPerfectZoom(currentViewportDisplayScale(), 1),
        );
      }
      break;
    }
    ...
    case "zoom-reset": {
      const anchor = viewportTransformOrigin();
      if (anchor !== undefined) zoomViewportAtClient(anchor.x, anchor.y, 1);
      break;
    }
    case "add-frame":
...
void resolveInitialProjectSettings().then(async (settings) => {
  ...
  // the actual desktop/tablet/mobile profile after the first project render.
