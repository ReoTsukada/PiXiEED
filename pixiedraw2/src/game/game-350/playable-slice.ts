...
function camera2DSettingsForProject(
  project: GameProject,
  scene: GameProject["scenes"][number],
): GameCamera2DSettings {
  const editorCamera = project.editorTimeline?.tracks
    .flatMap((track) => track.components ?? [])
    .find((component) => component.type === "CAMERA");
  const sceneCamera = scene.entities
    .flatMap((entity) => entity.components)
    .find((component) => component.type === "CAMERA");
  return normalizeCamera2DSettings(
    editorCamera?.camera2D ?? sceneCamera?.camera2D,
  );
}
...
function camera(componentId: string) {
  return {
    ...
    camera2D: {
      ...DEFAULT_CAMERA_2D_SETTINGS,
      follow: { ...DEFAULT_CAMERA_2D_SETTINGS.follow },
