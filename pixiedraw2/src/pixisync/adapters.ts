/** PIXISYNC-DRAW2-110 canonical, transport-free domain adapters. */
import {
...
import { createPixisyncDraft } from "./core.ts";
import type {
...
export type PixisyncAdapterErrorCode =
  | "IDENTITY_MISMATCH"
  | "PROJECT_MISMATCH"
  | "ASSET_MISMATCH"
...
export class PixisyncAdapterError extends Error {
  readonly code: PixisyncAdapterErrorCode;
  readonly path: string | undefined;
  constructor(code: PixisyncAdapterErrorCode, message: string, path?: string) {
    super(message);
    this.name = "PixisyncAdapterError";
    this.code = code;
    this.path = path;
  }
}
...
function fail(
  code: PixisyncAdapterErrorCode,
  message: string,
  path?: string,
): never {
  throw new PixisyncAdapterError(code, message, path);
}
...
function jsonObject(value: unknown, path: string): PixisyncJsonObject {
  if (!isRecord(value)) {
    fail("INVALID_COMMAND_PAYLOAD", "Object payload required.", path);
  }
  assertJson(value, path);
  return value as PixisyncJsonObject;
}
...
function decodeDraw(operation: PixisyncCommittedOperation) {
...
export function createDrawPixisyncAdapter(
...
export async function createAudioOperationDraft(
  ...
  if (
    entry.command.baseProjectRevision !== input.identity.baseProjectRevision
  ) {
    fail(
      "STALE_BASE",
      "Audio command base revision differs from the sync identity.",
      "baseProjectRevision",
    );
  }
  const payload = jsonObject({
...
function decodeAudio(operation: PixisyncCommittedOperation) {
  const payload = recordOf(operation.payload, "payload");
...
export function createAudioPixisyncAdapter(
...
export interface GameCanonicalApplyPort {
  ...
  appendJournalCommand(
    input: { readonly next: GameProject; readonly commandId: string },
...
function decodeGame(operation: PixisyncCommittedOperation) {
  const payload = recordOf(operation.payload, "payload");
  if (
...
export function createGamePixisyncAdapter(
...
export type Draw110AdapterDraft = PixisyncOperationDraft;
export type Audio110AdapterDraft = PixisyncOperationDraft;
export type Game110AdapterDraft = PixisyncOperationDraft;
