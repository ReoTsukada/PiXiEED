/** Deterministic validation and hashing for PIXISYNC-DRAW2-100. */

import { canonicalJson, sha256Hex } from "../wp160-contracts.ts";
import {
  PIXISYNC_DRAW2_SCHEMA_VERSION,
  type PixisyncAggregate,
  type PixisyncCommittedOperation,
  type PixisyncCompensationGuard,
  PixisyncError,
  type PixisyncJsonObject,
  type PixisyncJsonValue,
  type PixisyncOperationDraft,
  type PixisyncRevisionReference,
} from "./contracts.ts";

export const PIXISYNC_DRAW2_MAX_PAYLOAD_BYTES = 16_384;
export const PIXISYNC_DRAW2_MAX_PAYLOAD_DEPTH = 8;
export const PIXISYNC_DRAW2_MAX_PAYLOAD_KEYS = 96;
...
function reject(
  code: ConstructorParameters<typeof PixisyncError>[0],
  message: string,
  path?: string,
): never {
...
function assertAggregate(
  value: unknown,
  path: string,
): asserts value is PixisyncAggregate {
...
function assertJsonValue(
  ...
  depth: number,
  path: string,
): asserts value is PixisyncJsonValue {
...
function assertRevisionReference(
  value: unknown,
  path: string,
): asserts value is PixisyncRevisionReference {
  if (
    !isRecord(value) ||
...
export async function createPixisyncDraft(
...
export async function validatePixisyncDraft(
  draft: PixisyncOperationDraft,
): Promise<void> {
  if (
    !isRecord(draft) || draft.schemaVersion !== PIXISYNC_DRAW2_SCHEMA_VERSION
  ) {
    reject(
      "INVALID_ENVELOPE",
  ...
  const expectedHash = await payloadHash(draft.payload as PixisyncJsonObject);
...
function assertCompensation(
  value: unknown,
): asserts value is PixisyncCompensationGuard {
  if (!isRecord(value)) {
    reject(
      "INVALID_ENVELOPE",
...
export async function operationFingerprint(
  operation: PixisyncOperationDraft | PixisyncCommittedOperation,
): Promise<string> {
...
export function writerGuard(operation: PixisyncCommittedOperation): string {
  return `${operation.aggregate}:${operation.aggregateRevision}:${operation.operationId}`;
}

export async function validatePixisyncCommitted(
  operation: PixisyncCommittedOperation,
): Promise<void> {
...
export function revisionReferences(
  payload: PixisyncJsonObject,
): readonly PixisyncRevisionReference[] {
  const references: PixisyncRevisionReference[] = [];
  const visit = (value: unknown): void => {
    ...
    for (const [key, child] of Object.entries(value)) {
      if (key === "revisionRef") {
        references.push(child as PixisyncRevisionReference);
      } else visit(child);
    }
  };
  visit(payload);
