...
import type { PixisyncSnapshotPersistencePort } from "./durability.ts";

export type PixisyncProjectLifecyclePhase =
  | "IDLE"
  | "OPENING"
  | "ACTIVE"
  | "SWITCHING"
...
export interface PixisyncProjectJournal {
  readonly projectId: string;
}

export interface PixisyncProjectLifecycleState {
  readonly phase: PixisyncProjectLifecyclePhase;
  readonly projectId: string | null;
  readonly generation: number;
  readonly reason?: "INITIAL" | "OPEN" | "NEW" | "OPEN_FAILED" | "DISPOSE";
}

export interface PixisyncProjectLifecycleOptions<
  TJournal extends PixisyncProjectJournal,
> {
  ...
  readonly onState?: (state: PixisyncProjectLifecycleState) => void;
}
...
export class PixisyncProjectLifecycleCoordinator<
  TJournal extends PixisyncProjectJournal,
> {
  readonly #options: PixisyncProjectLifecycleOptions<TJournal>;
  #state: PixisyncProjectLifecycleState = {
    phase: "IDLE",
    projectId: null,
    generation: 0,
  };
  ...
  constructor(options: PixisyncProjectLifecycleOptions<TJournal>) {
    if (
  ...
  #publish(state: PixisyncProjectLifecycleState): void {
    this.#state = { ...state };
