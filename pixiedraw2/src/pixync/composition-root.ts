...
import type { PixyncAggregateAdapter, PixyncOperationDraft } from "./contracts.ts";
import { PixyncDurableJournal, type PixyncSnapshotPersistencePort } from "./durability.ts";
...
import { createPixyncSupabaseSdkPort, type PixyncSupabaseSdkClient } from "./supabase-sdk-port.ts";
...
export class PixyncProductionCompositionRoot {
  ...
  readonly #drawCommit = (event: Event): void => {
    const detail = customDetail<{ result: CommandResult; nextState: ProjectState; baseStructureEpoch: number }>(event);
    if (detail !== undefined && this.#products.submitDraw !== undefined) {
      void this.#products.submitDraw(detail).catch((error) => this.#report(error));
    }
  };
  readonly #audioCommit = (event: Event): void => {
    const entry = customDetail<{ readonly entry?: AudioJournalEntry }>(event)?.entry;
    if (entry !== undefined && this.#products.submitAudio !== undefined) {
      void this.#products.submitAudio(entry).catch((error) => this.#report(error));
    }
  };
  readonly #gameCommit = (event: Event): void => {
    ...
    if (detail !== undefined && this.#products.submitGameEditor !== undefined) {
      void this.#products.submitGameEditor(detail).catch((error) => this.#report(error));
    }
  };
  readonly #audioCatalog = (): void => {
    void this.#lazy.demandAudio("DRAW_AUDIO_LANE").catch((error) => this.#report(error));
  };
  readonly #audioPlayback = (event: Event): void => {
    if (customDetail<{ readonly playing?: boolean }>(event)?.playing === true) {
      void this.#lazy.demandAudio("DRAW_PLAYBACK").catch((error) => this.#report(error));
  ...
  readonly #audioExport = (event: Event): void => {
    if (customDetail<{ readonly active?: boolean }>(event)?.active === true) {
      void this.#lazy.demandAudio("DRAW_EXPORT").catch((error) => this.#report(error));
    } else this.#lazy.releaseAudio("DRAW_EXPORT");
  };
  readonly #gameCatalog = (): void => {
    void this.#lazy.demandGame("GAME_CATALOG").catch((error) => this.#report(error));
  };
  readonly #gameEditor = (event: Event): void => {
    if (customDetail<{ readonly active?: boolean }>(event)?.active === true) {
      void this.#lazy.demandGame("GAME_EDITOR").catch((error) => this.#report(error));
    } else this.#lazy.releaseGame("GAME_EDITOR");
  };
  readonly #gameBuild = (event: Event): void => {
    if (customDetail<{ readonly active?: boolean }>(event)?.active === true) {
      void this.#lazy.demandGame("GAME_BUILD").catch((error) => this.#report(error));
  ...
  static async create(options: PixyncProductionCompositionOptions): Promise<PixyncProductionCompositionRoot> {
    const transport = new PixyncTransportAdapter(new PixyncSupabaseProvider({
      port: createPixyncSupabaseSdkPort(options.supabase),
    }));
    const journal = await PixyncDurableJournal.open(options.projectId, options.persistence);
    ...
    const productTransport: PixyncProductionTransportPort = {
      get binding() { return transport.binding; },
    ...
    if (products.adapters.length !== 3 || aggregates.size !== 3 ||
      !aggregates.has("draw") || !aggregates.has("audio") || !aggregates.has("game")) {
    ...
    return new PixyncProductionCompositionRoot({ options, transport, coordinator, lazy, products });
  }

  get binding(): PixyncTransportBinding | undefined { return this.#transport.binding; }
  ...
  async connect(): Promise<void> {
    ...
    await this.#coordinator.connect({
      ...
      ...(this.#options.onError === undefined ? {} : { onCatchUpError: this.#options.onError }),
    });
    await this.#coordinator.reconcile();
    await this.#coordinator.catchUp();
    const binding = this.#transport.binding;
    if (binding === undefined) throw new Error("Authenticated PiXYNC binding is unavailable.");
    this.#eventTarget.dispatchEvent(new CustomEvent("draw2:pixync-binding", {
      detail: { projectId: binding.projectId, actorId: binding.actorId, clientId: binding.clientId, role: binding.role },
    }));
    this.#installEvents();
    this.#connected = true;
  }
  ...
  #report(error: unknown): void { this.#options.onError?.(error); }
