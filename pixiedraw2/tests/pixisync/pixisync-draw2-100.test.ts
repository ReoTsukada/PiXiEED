...
const PROJECT_ID = "pixisync-draw2-fixture";
type ApplyRecord = {
...
function adapters(
  applied: ApplyRecord[],
  failAggregate?: PixisyncAggregate,
): readonly PixisyncAggregateAdapter[] {
...
async function expectCode(
  ...
  assert.ok(caught instanceof PixisyncError);
  assert.equal(caught.code, code);
}

Deno.test("PIXISYNC-DRAW2-100-ENVELOPE rejects hash mutation and forbidden command payloads", async () => {
  const valid = await draft("op-envelope", "draw", {
    command: "stroke",
    cells: [1, 2, 3],
  });
  await expectCode(
    () => validatePixisyncDraft({ ...valid, payload: { command: "mutated" } }),
...
Deno.test("PIXISYNC-DRAW2-100-SEQUENCE converges two clients with one project revision authority", async () => {
  ...
  > = [];
  const transport = new PixisyncInMemoryTransport();
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT_ID,
  ...
  const clientA = new PixisyncOrderKeeper({
    projectId: PROJECT_ID,
    adapters: adapters(clientAApplied),
  });
  const clientB = new PixisyncOrderKeeper({
    projectId: PROJECT_ID,
...
Deno.test("PIXISYNC-DRAW2-100-ORDER rejects payload conflict and preserves exact idempotency", async () => {
...
Deno.test("PIXISYNC-DRAW2-100-ORDER rejects a committed operation ID moved to another server revision", async () => {
  const applied: ApplyRecord[] = [];
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT_ID,
    adapters: adapters(applied),
  ...
  const client = new PixisyncOrderKeeper({
    projectId: PROJECT_ID,
    adapters: adapters([]),
  });
...
Deno.test("PIXISYNC-DRAW2-100-COMPENSATE requires a current guard and emits a new operation", async () => {
  const applied: ApplyRecord[] = [];
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT_ID,
    adapters: adapters(applied),
...
Deno.test("PIXISYNC-DRAW2-100-CROSS-DOMAIN accepts locked Draw and Audio references in Game", async () => {
  const applied: ApplyRecord[] = [];
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT_ID,
    adapters: adapters(applied),
...
Deno.test("PIXISYNC-DRAW2-100-ATOMIC-FAILURE does not advance revision when adapter apply fails", async () => {
  const applied: ApplyRecord[] = [];
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT_ID,
...
Deno.test("PIXISYNC-DRAW2-100-REMOTE remote apply uses adapter only and does not provide an undo surface", async () => {
...
const _committedTypeCheck: PixisyncCommittedOperation | undefined = undefined;
