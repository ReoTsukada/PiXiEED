...
import {
  ...
  createAudioPixisyncAdapter,
  createDrawOperationDraft,
  createDrawPixisyncAdapter,
  createGameOperationDraft,
  createGamePixisyncAdapter,
  type DrawCanonicalApplyPort,
  drawRasterHash,
  type GameCanonicalApplyPort,
  PixisyncAdapterError,
} from "../../src/pixisync/adapters.ts";
import {
  createPixisyncDraft,
...
const PROJECT = "pixisync-draw2-110";
...
Deno.test("PIXISYNC-DRAW2-110-AUDIO-CONVERGENCE and AUDIO-BOUNDARY use command-only journal drafts", async () => {
  ...
  const adapter = createAudioPixisyncAdapter(makePort());
...
Deno.test("PIXISYNC-DRAW2-110-GAME-CONVERGENCE and GAME-BOUNDARY resolve then append canonical revisions", async () => {
  ...
  const adapter = createGamePixisyncAdapter(makePort());
  const sequencer = new PixisyncInMemorySequencer({
    projectId: PROJECT,
  ...
  const bad = createGamePixisyncAdapter(makePort(true));
...
Deno.test("PIXISYNC-DRAW2-110-ORDER, IDENTITY, and ATOMIC-FAILURE preserve existing core sequencing", async () => {
  const applied: string[] = [];
  const accepting = (aggregate: PixisyncAggregate): PixisyncAggregateAdapter =>
  ...
  const failing = new PixisyncInMemorySequencer({
    projectId: "fail-110",
    adapters: [
      {
        aggregate: "draw",
...
Deno.test("PIXISYNC-DRAW2-110 identity substitution rejects before canonical apply", async () => {
  const fixture = await drawFixture();
  await expectCode(() =>
    createDrawOperationDraft({
