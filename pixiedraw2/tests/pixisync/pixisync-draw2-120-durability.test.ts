...
import { committedOperationFingerprint } from "../../src/pixisync/core.ts";
import {
  PixisyncDurabilityCrashError,
  type PixisyncDurabilityCrashPoint,
  PixisyncDurabilityError,
  type PixisyncDurabilityOptions,
  PixisyncDurableJournal,
  type PixisyncDurableSnapshot,
  type PixisyncIncomingIdentity,
...
const PROJECT = "pixisync-draw2-120";
let time = Date.parse("2026-08-23T00:00:00.000Z");
const now = () => new Date(time);

async function draft(
  operationId: string,
  aggregate: PixisyncAggregate = "draw",
  clientSequence = 1,
...
): Promise<PixisyncCommittedOperation> {
  const value = await createPixisyncDraft({
    operationId,
    projectId: PROJECT,
    aggregate,
    actorId: "actor-120",
...
): Promise<PixisyncIncomingIdentity> {
  return {
...
async function open(
  persistence: PixisyncSnapshotPersistencePort =
    new PixisyncInMemorySnapshotPersistence(),
  options: PixisyncDurabilityOptions = {},
) {
  return PixisyncDurableJournal.open(PROJECT, persistence, { now, ...options });
}

function crashAt(point: PixisyncDurabilityCrashPoint) {
  let armed = true;
  return (actual: PixisyncDurabilityCrashPoint) => {
    if (armed && actual === point) {
      armed = false;
      throw new PixisyncDurabilityCrashError(actual);
    }
  };
}
...
function adapter(applied: string[], fail = false) {
  return {
    ...
    apply: (operation: PixisyncCommittedOperation) => {
...
async function receiveJournal(
  ...
  assert.ok(lease);
  const keeper = new PixisyncOrderKeeper({
    projectId: PROJECT,
...
Deno.test("PIXISYNC-DRAW2-120-01 enqueue restart resend", async () => {
  time = Date.parse("2026-08-23T00:00:00.000Z");
  const persistence = new PixisyncInMemorySnapshotPersistence();
...
Deno.test("PIXISYNC-DRAW2-120-02 duplicate fingerprint is idempotent", async () => {
...
Deno.test("PIXISYNC-DRAW2-120-03 same operation ID different fingerprint conflicts", async () => {
  const journal = await open();
  await journal.enqueue(await draft("conflict"));
  await assert.rejects(
    async () =>
      journal.enqueue(
        await createPixisyncDraft({
...
Deno.test("PIXISYNC-DRAW2-120-03b Outbox ACK conflicts with an Inbox revision", async () => {
  const journal = await open();
...
Deno.test("PIXISYNC-DRAW2-120-04 stale lease fencing rejects old token", async () => {
  time = Date.parse("2026-08-23T00:00:00.000Z");
...
Deno.test("PIXISYNC-DRAW2-120-05 ACK loss resend is idempotent", async () => {
  const persistence = new PixisyncInMemorySnapshotPersistence();
...
Deno.test("PIXISYNC-DRAW2-120-06 unconfirmed draft cannot be deleted", async () => {
  const journal = await open();
...
Deno.test(
  "PIXISYNC-DRAW2-120-06b pre-persist crashes expose no partial enqueue",
  async () => {
    for (
      ...
      await assert.rejects(
        ...
        (error: unknown) =>
          error instanceof PixisyncDurabilityCrashError &&
...
Deno.test("PIXISYNC-DRAW2-120-07 persist response crash survives restart", async () => {
  const persistence = new PixisyncInMemorySnapshotPersistence();
...
Deno.test("PIXISYNC-DRAW2-120-08 incoming duplicate applies once", async () => {
  const journal = await open();
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof PixisyncDurabilityError &&
...
Deno.test("PIXISYNC-DRAW2-120-09 gap-held is replayed in project order", async () => {
  const journal = await open();
...
Deno.test("PIXISYNC-DRAW2-120-10 adapter failure is retryable", async () => {
...
Deno.test("PIXISYNC-DRAW2-120-11 apply-before-ack receipt recovers without reapply", async () => {
  const persistence = new PixisyncInMemorySnapshotPersistence();
  const crashing = await open(persistence, {
...
Deno.test("PIXISYNC-DRAW2-120-12 tampered restore rejects fingerprint", async () => {
  const persistence = new PixisyncInMemorySnapshotPersistence();
  const journal = await open(persistence);
  await journal.enqueue(await draft("tampered"));
  const snapshot = persistence.snapshot()!;
  const bad: PixisyncDurableSnapshot = {
    ...snapshot,
    vault: {
      ...snapshot.vault,
  ...
  await assert.rejects(() =>
    open(new PixisyncInMemorySnapshotPersistence(bad))
  );
  const badHash: PixisyncDurableSnapshot = {
    ...snapshot,
    snapshotHash: "0".repeat(64),
  };
  await assert.rejects(() =>
    open(new PixisyncInMemorySnapshotPersistence(badHash))
  );
  await assert.rejects(
    () =>
      createPixisyncDraft({
    ...
    (error: unknown) =>
      error instanceof PixisyncError && error.code === "PAYLOAD_FORBIDDEN",
  );
});

Deno.test("PIXISYNC-DRAW2-120-12b incoming identity and revision conflicts are durable", async () => {
  const journal = await open();
...
Deno.test("PIXISYNC-DRAW2-120-13 three aggregate revisions remain durable", async () => {
  const journal = await open();
  for (
  ...
  const snapshot = journal.snapshot();
  assert.equal(snapshot.schemaVersion, "PIXSYNC_DRAW2_DURABLE_SNAPSHOT_V1");
...
Deno.test("PIXISYNC-DRAW2-120-14 retry limit reaches DLQ and capability stays non-production", async () => {
...
Deno.test("PIXISYNC-DRAW2-120-15 atomicReplace failure leaves memory unchanged", async () => {
  let calls = 0;
  const persistence: PixisyncSnapshotPersistencePort = {
    load: () => undefined,
...
Deno.test("PIXISYNC-DRAW2-120-REGRESSION predecessor core envelope rejects mutation", async () => {
  const value = await draft("regression");
  await assert.rejects(
    () =>
      import("../../src/pixisync/core.ts").then(({ validatePixisyncDraft }) =>
        validatePixisyncDraft({ ...value, payload: { changed: true } })
      ),
    (error: unknown) =>
      error instanceof PixisyncError && error.code === "PAYLOAD_HASH_MISMATCH",
  );
});
