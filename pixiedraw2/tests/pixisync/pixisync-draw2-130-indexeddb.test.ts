...
Deno.test("PIXISYNC-DRAW2-130-01 reload/open preserves a project snapshot", async () => {
  const factory = new FakeIndexedDbFactory();
  const first = createPixisyncIndexedDbPersistence(
    "project-a",
    options(factory, "reload"),
  );
  const journal = await PixisyncDurableJournal.open("project-a", first);
  const before = journal.snapshot();
  await first.atomicReplace(await seal(before, 1));

  const reopened = createPixisyncIndexedDbPersistence(
    "project-a",
    options(factory, "reload"),
  );
  const restored = await PixisyncDurableJournal.open("project-a", reopened);
  assert.equal(restored.snapshot().revision, 1);
});

Deno.test("PIXISYNC-DRAW2-130-02 project IDs are isolated in one database", async () => {
  const factory = new FakeIndexedDbFactory();
  const a = createPixisyncIndexedDbPersistence(
    "project-a",
    options(factory, "isolation"),
  );
  const b = createPixisyncIndexedDbPersistence(
    "project-b",
    options(factory, "isolation"),
  );
  const journalA = await PixisyncDurableJournal.open("project-a", a);
  const snapshotA = await seal(journalA.snapshot(), 1);
  await a.atomicReplace(snapshotA);
  assert.equal(await b.load(), undefined);
  const journalB = await PixisyncDurableJournal.open("project-b", b);
  assert.equal(journalB.snapshot().projectId, "project-b");
  assert.equal((await a.load())?.projectId, "project-a");
});

Deno.test("PIXISYNC-DRAW2-130-03 failed readwrite transaction preserves previous snapshot", async () => {
  const factory = new FakeIndexedDbFactory();
  const persistence = createPixisyncIndexedDbPersistence(
    "project-a",
    options(factory, "failure"),
  );
  const journal = await PixisyncDurableJournal.open("project-a", persistence);
...
Deno.test("PIXISYNC-DRAW2-130-04 tampered snapshot is rejected by journal open", async () => {
  const factory = new FakeIndexedDbFactory();
  const dbName = "tamper";
  const persistence = createPixisyncIndexedDbPersistence(
    "project-a",
  ...
  await assert.rejects(
    PixisyncDurableJournal.open(
      "project-a",
      createPixisyncIndexedDbPersistence("project-a", options(factory, dbName)),
    ),
    /hash mismatch/u,
  );
});

Deno.test("PIXISYNC-DRAW2-130-05 same-instance atomicReplace calls complete in call order", async () => {
  const factory = new FakeIndexedDbFactory();
  const persistence = createPixisyncIndexedDbPersistence(
    "project-a",
...
Deno.test("PIXISYNC-DRAW2-130-06 constructor supports an injected factory without host IndexedDB", () => {
  const factory = new FakeIndexedDbFactory();
  assert.doesNotThrow(() =>
    createPixisyncIndexedDbPersistence(
      "project-a",
