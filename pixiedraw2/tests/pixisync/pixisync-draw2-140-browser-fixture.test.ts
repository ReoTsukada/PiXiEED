...
const html = await Deno.readTextFile(
  "assets/pixisync-indexeddb-browser-harness.html",
);
const bundle = await Deno.readTextFile(
  "assets/pixisync-indexeddb-browser-harness.js",
);

Deno.test("PIXISYNC-DRAW2-140-FIXTURE-01 serves a browser-loadable local harness", () => {
  assert.match(html, /fetch\("\.\/pixisync-indexeddb-browser-harness\.js"/u);
  assert.match(html, /new Blob\(\[source\], \{ type: "text\/javascript" \}\)/u);
  assert.match(html, /window\.__pixisyncDraw2BrowserResult/u);
  assert.match(bundle, /export\s*\{[\s\S]*runPixisyncIndexedDbBrowserHarness/u);
  assert.match(bundle, /indexedDB\.open/u);
  assert.match(bundle, /localStorage/u);
});

Deno.test("PIXISYNC-DRAW2-140-FIXTURE-02 exposes all required browser result checks", () => {
  for (
    const id of [
      "project-isolation",
      "reload-recovery",
  ...
  assert.match(bundle, /PIXISYNC-DRAW2-140-BROWSER-HARNESS-V1/u);
  assert.match(bundle, /OPEN_FAILED/u);
  assert.match(bundle, /Tampered snapshot was rejected/u);
});

Deno.test("PIXISYNC-DRAW2-140-FIXTURE-03 remains outside product composition", () => {
