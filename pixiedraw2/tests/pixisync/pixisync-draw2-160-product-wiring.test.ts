...
const lifecycle = await Deno.readTextFile("src/pixisync/project-lifecycle.ts");
...
const lifecycleWiring = section(
  entry,
  "async function startPixisyncProjectLifecycle",
  "function layerKind",
);
const metadataWriter = section(
  entry,
  "function projectPixisyncState",
  "async function startPixisyncProjectLifecycle",
);

Deno.test("PIXISYNC-DRAW2-160 uses the common workspace event and canonical startup ID", () => {
...
Deno.test("PIXISYNC-DRAW2-160 creates project-bound IndexedDB persistence and opens the durable journal", () => {
  assert.match(
    lifecycleWiring,
    /createPersistence:\s*\(projectId\)\s*=>\s*\n?\s*createPixisyncIndexedDbPersistence\(projectId\)/u,
  );
  assert.match(
    lifecycleWiring,
    /PixisyncDurableJournal\.open\(projectId,\s*persistence\)/u,
  );
  assert.match(entry, /from "\.\/pixisync\/indexeddb-persistence\.ts"/u);
  assert.match(entry, /from "\.\/pixisync\/durability\.ts"/u);
});

Deno.test("PIXISYNC-DRAW2-160 keeps page lifecycle boundaries local and bfcache-safe", () => {
  assert.match(
    entry,
...
Deno.test("PIXISYNC-DRAW2-160 exposes only bounded lifecycle metadata", () => {
...
Deno.test("PIXISYNC-DRAW2-160 does not connect transport or operation dispatch", () => {
  assert.doesNotMatch(
    entry,
    /from\s+["'][^"']*(?:supabase|realtime)[^"']*["']/iu,
  );
  assert.doesNotMatch(
    lifecycleWiring,
...
Deno.test("PIXISYNC-DRAW2-160 does not synchronize Draw/Audio/Game payloads or UI preferences", () => {
  assert.doesNotMatch(
    lifecycleWiring,
...
Deno.test("PIXISYNC-DRAW2-160 detects the Draw2 cache-buster without changing it", () => {
  ...
  console.info(
    `[PIXISYNC-DRAW2-160] cache-buster=${cacheBuster || "UNTESTED"}`,
  );
  // Observation only: this gate never edits index.html or rewrites the query.
  assert.equal(typeof cacheBuster, "string");
});

Deno.test("PIXISYNC-DRAW2-160 keeps the lifecycle coordinator transport-free", () => {
