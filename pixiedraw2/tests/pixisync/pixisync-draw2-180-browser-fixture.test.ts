...
const html = await Deno.readTextFile(
  "assets/pixisync-cross-tab-cas-browser-harness.html",
);
const bundle = await Deno.readTextFile(
  "assets/pixisync-cross-tab-cas-browser-harness.js",
);
...
const browserEvidence = JSON.parse(
  await Deno.readTextFile(
    "../docs/inventory/pixisync-draw2-180-browser-evidence.json",
  ),
) as {
...
Deno.test("PIXISYNC-DRAW2-180-FIXTURE-BOUNDARY-01 keeps substituted input rejected", () => {
...
Deno.test("PIXISYNC-DRAW2-180-FIXTURE-02 exposes a DOM-controlled browser bridge", () => {
  assert.match(html, /id="command"/u);
  assert.match(html, /id="bridge"/u);
  assert.match(html, /id="result"/u);
  assert.match(html, /pixisync-cross-tab-cas-browser-harness\.js/u);
});

Deno.test("PIXISYNC-DRAW2-180-FIXTURE-03 bundles the dedicated CAS API", () => {
  assert.match(bundle, /pixisync-draw2-180-qualification-/u);
...
Deno.test("PIXISYNC-DRAW2-180-FIXTURE-04 stays outside product composition", () => {
  assert.doesNotMatch(productEntry, /pixisync-cross-tab-cas-browser-harness/u);
  assert.doesNotMatch(bundle, /supabase|RealtimeChannel|BroadcastChannel/u);
});

Deno.test("PIXISYNC-DRAW2-180-FIXTURE-05 binds evidence to the dedicated database", () => {
  assert.equal(browserEvidence.packageId, "PIXISYNC-DRAW2-180");
  assert.equal(
    browserEvidence.databaseName,
    "pixisync-draw2-180-qualification-v1",
  );
  ...
  assert.match(browserEntry, /pixisync-draw2-180-qualification-/u);
  assert.ok(browserEntry.includes("PIXISYNC_DRAW2_180_DB_PREFIX}v1"));
  assert.match(browserEntry, /pixisync-draw2-180-/u);
});
