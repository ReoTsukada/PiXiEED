...
import { PixyncProjectLifecycleCoordinator } from "../../src/pixync/project-lifecycle.ts";
...
function fixture(open?: (projectId: string) => Promise<{ projectId: string }>) {
  ...
  const calls: string[] = [];
  const states: string[] = [];
  const coordinator = new PixyncProjectLifecycleCoordinator({
    eventTarget: target,
    eventName: EVENT,
    initialProjectId: "project-a",
    createPersistence(projectId) {
      calls.push(`persistence:${projectId}`);
      return new PixyncInMemorySnapshotPersistence();
    },
...
Deno.test("PIXYNC-DRAW2-150 initial open and ordered project switch", async () => {
  const f = fixture();
...
Deno.test("PIXYNC-DRAW2-150 same OPEN is no-op and NEW reopens", async () => {
  const f = fixture();
  await f.coordinator.start();
...
Deno.test("PIXYNC-DRAW2-150 stale intermediate open never becomes active", async () => {
  let releaseB!: () => void;
...
Deno.test("PIXYNC-DRAW2-150 failed open does not reuse old journal", async () => {
  const f = fixture(async (projectId) => {
...
Deno.test("PIXYNC-DRAW2-150 dispose removes listener and drains active journal", async () => {
  const f = fixture();
  await f.coordinator.start();
...
Deno.test("PIXYNC-DRAW2-150 rejects substituted journal identity", async () => {
...
Deno.test("PIXYNC-DRAW2-150 flush failure stays recoverable", async () => {
  const target = new EventTarget();
  let failFlush = true;
  const opened: string[] = [];
  const coordinator = new PixyncProjectLifecycleCoordinator({
    ...
    eventName: EVENT,
    initialProjectId: "project-a",
    createPersistence: () => new PixyncInMemorySnapshotPersistence(),
    async openJournal(projectId) {
