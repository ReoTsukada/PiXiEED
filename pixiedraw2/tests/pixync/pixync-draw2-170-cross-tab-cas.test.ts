...
const PROJECT = "pixync-draw2-170";
...
function persistence(
  ...
  return createPixyncIndexedDbPersistence(
    projectId,
...
type CasPersistence = PixyncSnapshotPersistencePort & {
  compareAndSwap(
    snapshot: PixyncDurableSnapshot,
...
async function compareAndSwap(
  persistencePort: PixyncSnapshotPersistencePort,
  snapshot: PixyncDurableSnapshot,
  expectedSnapshotHash: string | null,
): Promise<void> {
  ...
  assert.equal(
    ...
    "function",
    "PIXYNC-DRAW2-170 requires optional compareAndSwap on the implementation.",
  );
...
class CasOnlyPersistence implements PixyncSnapshotPersistencePort {
  ...
  compareAndSwap(
    ...
    if ((this.#snapshot?.snapshotHash ?? null) !== expectedSnapshotHash) {
      const error = new Error("PiXYNC snapshot compare-and-swap conflict.") as
        & Error
        & {
          code?: string;
        };
...
Deno.test("PIXYNC-DRAW2-170-01 CAS uses one readwrite transaction", async () => {
  const factory = new FakeIndexedDbFactory();
  const port = persistence(factory);
  const journal = await PixyncDurableJournal.open(PROJECT, port);
...
Deno.test("PIXYNC-DRAW2-170-02 same-base adapters have one CAS winner", async () => {
  const factory = new FakeIndexedDbFactory();
  const seed = persistence(factory);
  const base = (await PixyncDurableJournal.open(PROJECT, seed)).snapshot();
...
Deno.test("PIXYNC-DRAW2-170-03 different projects do not conflict", async () => {
  const factory = new FakeIndexedDbFactory();
  const projectA = persistence(factory, "project-a");
  const projectB = persistence(factory, "project-b");
  const baseA = (await PixyncDurableJournal.open("project-a", projectA))
    .snapshot();
  const baseB = (await PixyncDurableJournal.open("project-b", projectB))
    .snapshot();
...
Deno.test("PIXYNC-DRAW2-170-04 journal uses current hash and preserves stale memory", async () => {
  const factory = new FakeIndexedDbFactory();
  const firstPort = persistence(factory);
  await PixyncDurableJournal.open(PROJECT, firstPort);
  const first = await PixyncDurableJournal.open(
    PROJECT,
    persistence(factory),
  );
  const second = await PixyncDurableJournal.open(
    PROJECT,
    persistence(factory),
  );
...
Deno.test("PIXYNC-DRAW2-170-05 initial journal creation uses expected null", async () => {
  const persistencePort = new CasOnlyPersistence();
  const journal = await PixyncDurableJournal.open(PROJECT, persistencePort);
  assert.ok(journal.snapshot().snapshotHash.length > 0);
  assert.deepEqual(persistencePort.expectedHashes, [null]);
});

Deno.test("PIXYNC-DRAW2-170-06 transaction abort preserves storage and journal memory", async () => {
  const factory = new FakeIndexedDbFactory();
  const port = persistence(factory);
  const journal = await PixyncDurableJournal.open(PROJECT, port);
...
Deno.test("PIXYNC-DRAW2-170-07 journals without CAS retain legacy fallback", async () => {
  const persistencePort = new PixyncInMemorySnapshotPersistence();
  const journal = await PixyncDurableJournal.open(PROJECT, persistencePort);
