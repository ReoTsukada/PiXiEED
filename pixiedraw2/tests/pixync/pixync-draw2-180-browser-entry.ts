...
const PIXYNC_DRAW2_180_DB_PREFIX =
  "pixync-draw2-180-qualification-" as const;
const PIXYNC_DRAW2_180_DB_NAME = `${PIXYNC_DRAW2_180_DB_PREFIX}v1` as const;
const PIXYNC_DRAW2_180_PROJECT_PREFIX = "pixync-draw2-180-" as const;
const PIXYNC_DRAW2_180_API_KEY = "__pixyncDraw2CrossTabCas" as const;

type PixyncDraw2CrossTabCasOpenResult = {
  readonly journalId: string;
  readonly projectId: string;
  readonly snapshot: PixyncDurableSnapshot;
};

type PixyncDraw2CrossTabCasEnqueueInput = {
  readonly journalId: string;
  readonly baseSnapshotHash: string;
  readonly operationId?: string;
};

type PixyncDraw2CrossTabCasEnqueueResult =
  | {
    readonly ok: true;
    readonly code: "ENQUEUED";
    readonly journalId: string;
    readonly operationId: string;
    readonly journalSnapshot: PixyncDurableSnapshot;
  }
  | {
    readonly ok: false;
    ...
    readonly operationId: string;
    readonly journalSnapshot: PixyncDurableSnapshot;
    readonly persistentSnapshot: PixyncDurableSnapshot | null;
    readonly errorMessage: string;
  };

type PixyncDraw2CrossTabCasBrowserApi = Readonly<{
  resetTestDb(): Promise<{ readonly dbName: string; readonly deleted: true }>;
  openJournal(projectId: string): Promise<PixyncDraw2CrossTabCasOpenResult>;
  getCurrentSnapshot(journalId: string): Promise<PixyncDurableSnapshot>;
  enqueueFromBase(
    input: PixyncDraw2CrossTabCasEnqueueInput,
  ): Promise<PixyncDraw2CrossTabCasEnqueueResult>;
  readPersistentSnapshot(
    projectId: string,
  ): Promise<PixyncDurableSnapshot | null>;
}>;
...
function requireProjectId(value: unknown): asserts value is string {
  if (
    typeof value !== "string" ||
    !new RegExp(
      `^${PIXYNC_DRAW2_180_PROJECT_PREFIX}[A-Za-z0-9._:/-]{1,96}$`,
      "u",
    ).test(value)
  ) {
    throw new TypeError(
      `PiXYNC Draw2 180 qualification projectId must use ${PIXYNC_DRAW2_180_PROJECT_PREFIX}.`,
    );
  }
}
...
function requireOperationId(value: unknown): string {
  if (value === undefined) {
    nextOperationId += 1;
    return `pixync-draw2-180-op-${nextOperationId}`;
  }
  if (
    typeof value !== "string" ||
...
function deleteDedicatedTestDatabase(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const request = factory.deleteDatabase(PIXYNC_DRAW2_180_DB_NAME);
    request.onerror = () =>
      reject(
...
function persistentSnapshot(
  projectId: string,
): Promise<PixyncDurableSnapshot | undefined> {
  return createPixyncIndexedDbPersistence(projectId, {
    dbName: PIXYNC_DRAW2_180_DB_NAME,
  }).load();
}
...
async function resetTestDb(): Promise<{
  ...
  return { dbName: PIXYNC_DRAW2_180_DB_NAME, deleted: true };
}

async function openJournal(
  projectId: string,
): Promise<PixyncDraw2CrossTabCasOpenResult> {
  requireProjectId(projectId);
  const persistence = createPixyncIndexedDbPersistence(projectId, {
    dbName: PIXYNC_DRAW2_180_DB_NAME,
  });
  const journal = await PixyncDurableJournal.open(projectId, persistence);
  nextJournalId += 1;
  const journalId = `pixync-draw2-180-journal-${nextJournalId}`;
...
async function enqueueFromBase(
  ...
  const payload = {
    command: "pixync-draw2-180-qualification-enqueue",
    operationId,
  } as const;
  const draft: PixyncOperationDraft = await createPixyncDraft({
    operationId,
    projectId: handle.projectId,
    aggregate: "draw",
    actorId: "pixync-draw2-180-qualification",
    clientId: `pixync-draw2-180-client-${input.journalId}`,
    clientSequence: 1,
...
const api: PixyncDraw2CrossTabCasBrowserApi = Object.freeze({
  resetTestDb,
  openJournal,
  getCurrentSnapshot,
  enqueueFromBase,
      ...
      switch (command.method) {
        ...
        case "enqueueFromBase":
          value = await api.enqueueFromBase(
            command.args as PixyncDraw2CrossTabCasEnqueueInput,
          );
          break;
...
Object.defineProperty(globalThis, PIXYNC_DRAW2_180_API_KEY, {
  configurable: false,
  enumerable: false,
  value: api,
  writable: false,
...
const loadState = (globalThis as typeof globalThis & {
  __pixyncDraw2CrossTabCasLoad?: { state: string; error: string | null };
}).__pixyncDraw2CrossTabCasLoad;
...
declare global {
  interface Window {
    readonly __pixyncDraw2CrossTabCas: PixyncDraw2CrossTabCasBrowserApi;
  }
}
