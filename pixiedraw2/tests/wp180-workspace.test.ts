...
Deno.test("WP-180 capability profiles use Canvas-first mobile and adaptive tablet", () => {
  const mobile = resolvePresentationProfile({
    ...
    hover: false,
    touch: true,
    stylusCandidate: true,
    orientation: "portrait",
    safeArea: { top: 0, right: 0, bottom: 24, left: 0 },
  });
  const tablet = resolvePresentationProfile({
    width: 834,
    height: 1112,
    pointerCoarse: true,
    pointerFine: true,
  ...
  const tabletLandscape = resolvePresentationProfile({
    width: 1024,
    height: 768,
    pointerCoarse: true,
    pointerFine: true,
  ...
  const split = resolvePresentationProfile({
    width: 834,
    height: 600,
    pointerCoarse: true,
    pointerFine: true,
  ...
  const desktop = resolvePresentationProfile({
    width: 1280,
    height: 900,
    pointerCoarse: false,
    pointerFine: true,
  ...
  if (
    mobile.profile !== "mobile" || tablet.profile !== "tablet" ||
    tabletLandscape.profile !== "tablet" || split.profile !== "split" ||
    !split.splitView || desktop.profile !== "desktop"
  ) throw new Error("Capability profile mismatch");
});
...
Deno.test("WP-180 Tablet projection keeps one active context surface", () => {
  const portrait = projectTabletPresentation(resolvePresentationProfile({
    width: 834,
    height: 1112,
    pointerCoarse: true,
    pointerFine: true,
  ...
  const landscape = projectTabletPresentation(resolvePresentationProfile({
    width: 1024,
    height: 768,
    pointerCoarse: true,
    pointerFine: true,
  ...
  if (
    portrait?.contextSurface !== "split_deck" ||
    portrait.timeline !== "on_demand" || portrait.maxActiveContextPanels !== 1
  ) {
    throw new Error(
      `Portrait Tablet projection mismatch: ${JSON.stringify(portrait)}`,
    );
  }
  if (
    landscape?.contextSurface !== "compact_dock" ||
    landscape.timeline !== "resizable" || landscape.toolRail !== "shell_fixed"
  ) {
    throw new Error(
      `Landscape Tablet projection mismatch: ${JSON.stringify(landscape)}`,
    );
  }
});

Deno.test("WP-180 Tablet Adaptive Deck keeps 44px controls and one heavy surface", () => {
  if (TABLET_SQUARE_CONTROL_SIZE !== 44) {
    throw new Error("Tablet square controls must remain 44px");
  }
  if (
    TABLET_PORTRAIT_DECK_CONTROLS.join(",") !==
      "color,layers,timeline,tags,markers,audio"
  ) throw new Error("Tablet deck control reachability changed");
  if (resolveTabletDeckSurface(false) !== "split") {
    throw new Error("Default Tablet deck must show palette and context");
  }
  if (resolveTabletDeckSurface(true) !== "timeline") {
    throw new Error("Timeline must replace, not overlap, the split deck");
  }
});

Deno.test("WP-180 resize/orientation notifications are coalesced", () => {
...
Deno.test("WP-180 Tablet tool flyouts are hidden unless their group is open", () => {
...
Deno.test("PC Draw tool rail stays fixed and parent-only", async () => {
  ...
  if ((html.match(/data-mobile-only-left-dock-chrome/g) ?? []).length !== 4) {
    throw new Error("Desktop must identify every legacy mobile rail surface");
  }
});
