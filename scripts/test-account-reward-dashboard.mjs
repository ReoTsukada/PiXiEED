...
const pixfindOwnershipMigration = fs.readFileSync('supabase/migrations/20260724131224_pixfind_creator_account_management.sql', 'utf8');
...
assert.match(pixfindPosts, /PiXiEEDAccountShare\.copyLink\(shareUrl\)/);
assert.match(pixfindPosts, /https:\/\/pixieed\.jp\/pixfind\/\?puzzle=/);
assert.doesNotMatch(pixfindPosts, /https:\/\/pixieed\.jp\/pixfind\/puzzles\//);
assert.match(pixfindPosts, /account-card-action--delete/);
assert.match(pixfindPosts, /pixfind_claim_my_puzzles_v1/);
assert.match(pixfindPosts, /creator_user_id\.eq\.\$\{access\.userId\}/);
assert.match(pixfindOwnershipMigration, /for delete/);
assert.match(pixfindOwnershipMigration, /creator_user_id = auth\.uid\(\)/);
assert.match(pixfindOwnershipMigration, /client_id = public\.request_client_id\(\)/);
