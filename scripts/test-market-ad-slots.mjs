...
assert.match(marketHtml, /card-feed-ads\.js/);
assert.match(marketHtml, /<script defer src="market\.js\?v=20260730-progressive-feed1">/);
...
assert.match(marketJs, /PiXiEEDCardFeedAds\?\.renderProgressively/);
assert.match(marketJs, /createAd: \(\) => window\.PiXiEEDMarketAds\?\.createListAd\?\.\(\)/);
assert.match(marketJs, /requestAd: \(ad\) => window\.PiXiEEDMarketAds\?\.observe\?\.\(ad\)/);
assert.match(adsJs, /const LIST_SLOT = window\.PiXiEEDCardFeedAds\?\.COMMON_SLOT \|\| '2141591954'/);
...
assert.match(adsJs, /Object\.freeze\(\{ createListAd, observe, showDetailAd \}\)/);
...
assert.match(css, /\.market-ad--list\s*\{[\s\S]*?height:\s*var\(--pixieed-card-feed-ad-height, 180px\)/);
