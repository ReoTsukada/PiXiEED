...
assert.equal(await detectFormat(file('music.mp3', [1, 2, 3], 'audio/mpeg')), 'mp3');
assert.equal(await detectFormat(file('music.wav', [1, 2, 3], 'audio/wav')), 'wav');
assert.equal(detectMediaKind(file('music.mp3', [1, 2, 3], 'audio/mpeg'), 'mp3'), 'audio');
assert.equal(detectMediaKind(file('music.wav', [1, 2, 3], 'audio/wav'), 'wav'), 'audio');
assert.equal(detectMediaKind(file('hero.png', png, 'image/png'), 'png'), 'image');
...
const audioMigration = fs.readFileSync(new URL('../supabase/migrations/20260824202450_market_audio_product_composition.sql', import.meta.url), 'utf8');
const optionPricingMigration = fs.readFileSync(new URL('../supabase/migrations/20260721120000_market_single_price_listings.sql', import.meta.url), 'utf8');
const sharedAd = fs.readFileSync(new URL('./bottom-nav-footer-ad.js', import.meta.url), 'utf8');
assert.match(sell, /bindLocalUi\(\);[\s\S]*initRemote\(\);/);
...
assert.match(sell, /audio-only/);
assert.match(sell, /image-audio/);
assert.match(sell, /AUDIO_FORMATS/);
assert.match(sell, /512×512pxを超える画像素材/);
assert.match(sellHtml, /id="listingDropZone"/);
assert.match(sellHtml, /id="listingPreviewGrid"/);
assert.doesNotMatch(sellHtml, /id="listingOptionDetails"/);
assert.doesNotMatch(sellHtml, /id="listingOptionPriceFields"/);
assert.match(sellHtml, /id="listingSourcePicker"/);
assert.match(sellHtml, /id="listingSourceDialog"/);
assert.match(sellHtml, /id="listingDeliveryManifestSection"/);
assert.match(sellHtml, /id="listingDeliverySummary"/);
assert.doesNotMatch(sellHtml, /id="listingTotalPrice"/);
...
assert.match(sell, /market_create_root_asset_v8/);
...
assert.match(item, /iDRAW \+ iAUDIO素材/);
assert.match(item, /iAUDIO素材（音楽・SE）/);
const accountPurchases = fs.readFileSync(new URL('./account-market-purchases.js', import.meta.url), 'utf8');
assert.match(accountPurchases, /mp3: 'MP3'/);
assert.match(accountPurchases, /iAUDIO素材/);
assert.match(accountPurchases, /hasAudio/);
...
assert.match(optionPricingMigration, /500 yen and use 100 yen increments/i);
assert.match(audioMigration, /'mp3'/);
assert.match(audioMigration, /media_kind in \('project', 'image', 'animation', 'audio'\)/);
