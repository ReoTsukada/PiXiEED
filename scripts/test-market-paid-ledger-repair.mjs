...
const sellerReconcile = fs.readFileSync('supabase/functions/market-reconcile-my-sales/index.ts', 'utf8');
...
assert.match(sellerReconcile, /requireMarketUser\(request\)/);
assert.match(sellerReconcile, /asset\.creator_user_id", user\.id/);
assert.match(sellerReconcile, /session\.payment_status !== "paid"/);
assert.match(sellerReconcile, /market_complete_stripe_purchase_v1/);
