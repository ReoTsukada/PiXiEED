...
assert.match(checkoutFunction, /grossAmount < 100/);
...
assert.match(sellUi, /purchasePrice < MIN_PAID_PRICE_YEN/);
assert.match(sellUi, /limitedOptionPrice/);
assert.match(limitedMigration, /limited_quantity integer/i);
assert.match(optionPricingMigration, /limited sale option price must be at least 100 yen and use 100 yen increments/i);
assert.match(optionPricingMigration, /market_assets_limited_minimum_price/i);
...
assert.match(optionPricingMigration, /market listing price must be at least 100 yen/i);
assert.match(optionPricingMigration, /market purchase amount must be at least 100 yen/i);
assert.match(optionPricingMigration, /market_assets_published_minimum_price/i);
...
assert.match(sellUi, /market_create_root_asset_v7/);
...
assert.match(optionPricingMigration, /create or replace function public\.market_create_root_asset_v7/i);
assert.match(optionPricingMigration, /custom option prices must be between 100 and 10000000 yen in 100 yen increments/i);
assert.match(sellUi, /market_create_derivative_draft_v4/);
assert.match(sellUi, /market_derivative_listing_context_v1/);
assert.doesNotMatch(sellUi, /\$\('listingOptionsSection'\)\.disabled = true/);
assert.match(sellUi, /\$\('listingDerivativeAllowed'\)\.disabled = true/);
assert.match(accountPurchases, /派生作品を出品/);
