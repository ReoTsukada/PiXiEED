...
const draw2Entry = fs.readFileSync('pixiedraw2/src/draw2-entry.ts', 'utf8');
const draw2LegacyCompat = fs.readFileSync('pixiedraw2/src/draw2-legacy-compat.ts', 'utf8');
...
assert.match(itemHtml, /id="itemGameUse"/);
assert.match(itemHtml, /id="itemUseInGame"/);
assert.match(itemHtml, /market-purchase-delivery\.js/);
assert.match(itemUi, /market-download/);
assert.match(itemUi, /stagePiXiEEDrawFile/);
assert.match(itemUi, /market_record_asset_binding_v1/);
assert.match(itemUi, /mode: 'GAME_BIND'/);
assert.match(itemUi, /MARKET_GAME_TARGET_KEY/);
...
assert.match(accountPurchases, /const canOpenInDraw = formats\.includes\('pixiedraw-project'\)/);
assert.match(accountPurchases, /kind: 'pixieedraw-open'/);
assert.match(accountPurchases, /stagePiXiEEDrawFile/);
assert.match(accountPurchases, /pixiedraw2\/\?market_import=/);
...
assert.match(draw2Entry, /pixieed-market-import-v1/);
assert.match(draw2Entry, /market_import/);
assert.match(draw2Entry, /consumeMarketImportRecord/);
assert.match(draw2Entry, /source: "MARKET_PURCHASE"/);
assert.match(draw2Entry, /new File\(\s*\[record\.blob\]/s);
assert.match(draw2Entry, /importPxdFile\(file,\s*\{[\s\S]*source: "MARKET_PURCHASE"/);
assert.match(draw2Entry, /MARKET_IMPORT_FILENAME_PATTERN/);
assert.match(draw2Entry, /store\.delete\(token\)/);
assert.match(draw2Entry, /PXD import rejected/);
assert.match(draw2LegacyCompat, /inspectPxd/);
...
assert.match(`${sellUi}\n${secureDeliveryFunction}\n${accountPurchases}`, /pixiedraw-project/);
assert.match(marketUi, /iDRAW作品/);
...
assert.match(sellUi, /purchasePrice !== 0 && purchasePrice < 500/);
...
assert.match(sellHtml, /id="listingRightsLabel"[^>]*>この作品を出品し、設定した範囲で利用許諾できる権限/);
...
assert.match(sharedNav, /key: 'market'[\s\S]*path: 'market\/'/);
...
assert.match(marketUi, /PiXiEEDMarketAccess\.getClient/);
assert.match(marketUi, /rpc\('market_public_catalog_v1'/);
assert.match(marketUi, /void loadPreviewUrls/);
assert.match(marketUi, /void Promise\.resolve\(favorites\?\.prepare\?\./);
assert.doesNotMatch(marketUi, /await loadPreviewUrls/);
assert.doesNotMatch(marketUi, /from\('market_assets'\)/);
assert.match(itemUi, /rest\/v1\/rpc\/market_public_asset_v1/);
