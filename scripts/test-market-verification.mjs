...
assert.match(sellUi, /purchasePrice < MIN_LISTING_PRICE_YEN/);
assert.match(sellUi, /limitedOptionPrice/);
assert.match(limitedMigration, /limited_quantity integer/i);
assert.match(singlePriceMigration, /option_price_yen', 0/i);
...
assert.match(singlePriceMigration, /market listing price must be at least 500 yen/i);
assert.match(singlePriceMigration, /market purchase amount must be at least 500 yen/i);
assert.match(singlePriceMigration, /market_assets_published_minimum_price/i);
...
assert.match(sellUi, /market_create_root_asset_v8/);
...
assert.match(singlePriceMigration, /create or replace function public\.market_create_root_asset_v8/i);
assert.match(singlePriceMigration, /seller-priced custom options are not available/i);
assert.match(sellUi, /market_create_derivative_draft_v5/);
