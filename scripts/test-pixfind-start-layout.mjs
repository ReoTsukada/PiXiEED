...
assert.match(html, /app\.js\?v=20260728-individual-ogp-share1/);
...
assert.match(html, /id="targetPanelSecondary"/);
assert.match(html, /id="targetListSecondary"/);
assert.match(html, /class="result-panel" role="dialog"/);
assert.match(html, /id="resultRetryButton"/);
assert.match(html, /id="creatorCompleteOverlay"/);
assert.match(html, /id="creatorCompletePlay"/);
assert.match(html, /id="creatorCompleteShare"/);
assert.match(html, /id="creatorCompleteCopy"/);
assert.match(html, /id="creatorCompleteShareStatus"/);
...
assert.match(app, /Creator markers and target labels are paired by their creation order\./);
assert.match(app, /\? diffResult\.regions\.slice\(0, HIDDEN_OBJECT_TARGET_LIMIT\)/);
assert.match(app, /function updatePixfindTabBarActions\(screen\)/);
assert.match(app, /Difference detection produces rectangular bounds,[\s\S]*context\.arc\(centerX, centerY, radius, 0, Math\.PI \* 2\)/);
assert.match(app, /offscreen\.getContext\('2d', \{ willReadFrequently: true \}\)/);
assert.match(app, /function drawCreatorDiffPreviewOverlay\(\)/);
assert.match(app, /creatorDiffPreviewOverlay/);
assert.match(app, /overlay\.removeAttribute\('hidden'\)/);
assert.match(css, /\.creator-diff-preview-overlay \{[\s\S]*pointer-events: none;/);
assert.match(app, /function openCreatorEditFromUrl\(\)/);
assert.match(app, /function openCreatorCompletePanel\(puzzle, \{ isEditing = false \} = \{\}\)/);
assert.match(app, /function copyCreatorPublishedLink\(\)/);
assert.match(app, /creatorEditPost/);
...
assert.match(app, /function prepareCreatorPublishedOgpShare\(puzzle\)/);
...
assert.match(app, /title\.textContent = hasPreview \? '画像を変更'/);
...
assert.match(css, /\.creator-complete-panel \{/);
...
assert.match(app, /function renderSuccessBurst\(canvas, imageX, imageY, region\)/);
...
assert.match(css, /\.hit-burst__piece/);
