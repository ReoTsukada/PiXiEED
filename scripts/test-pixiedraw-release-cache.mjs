...
assert.match(indexHtml, /assets\/css\/style\.css\?v=20260828-project-list-window1/);
assert.match(indexHtml, /scripts\/card-feed-ads\.js\?v=20260803-leading-project-ad1/);
assert.match(indexHtml, /startup-workflow-utils\.js\?v=20260828-project-list-window1/);
assert.match(indexHtml, /shared-bottom-nav\.js\?v=20260825-studio-brand1/);
...
assert.match(indexHtml, /autosave-schema-v2-indexeddb-utils\.js\?v=20260828-delete-barrier1/);
assert.match(indexHtml, /project-storage-utils\.js\?v=20260828-recent-delete-tombstone1/);
for (const moduleName of [
  ...
  const cacheVersion = moduleName === 'pixisync-realtime-client-utils'
    ? '20260805-pixisync-members-profile1'
    : '20260802-pixisync-lazy-cell1';
  assert.match(indexHtml, new RegExp(`${moduleName}\\.js\\?v=${cacheVersion}`));
}
for (const moduleName of [
...
for (const assetName of [
  ...
  assert.match(indexHtml, new RegExp(`${assetName}\\?v=20260805-pixisync-members-profile1`));
}
for (const assetName of [
  ...
  const cacheVersion = assetName.includes('open-import-workflow-utils')
    ? '20260828-project-list-light1'
    : (assetName.includes('document-session-workflow-utils')
      ? '20260803-project-conversion-no-dialog1'
      : (assetName.includes('pixisync-project-switch-utils')
        ? '20260803-pixisync-code-only1'
        : '20260802-project-share-scope1'));
  assert.match(indexHtml, new RegExp(`${assetName}\\?v=${cacheVersion}`));
}
assert.match(indexHtml, /assets\/js\/modules\/pixisync-runtime-adapter-utils\.js\?v=20260805-pixisync-members-profile1/);
assert.match(indexHtml, /assets\/js\/app\.js\?v=20260828-delete-guard1/);
