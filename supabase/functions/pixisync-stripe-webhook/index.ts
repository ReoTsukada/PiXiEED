...
serve(async (request) => {
  ...
  if (checkoutEvent && metadata.pixieed_product_key !== "pixisync_project_slot") {
  ...
  if (chargeId) {
    ...
    if (!data) return jsonResponse(request, { ok: true, ignored: true });
  }
  if (!checkoutEvent && !chargeId) return jsonResponse(request, { ok: true, ignored: true });
