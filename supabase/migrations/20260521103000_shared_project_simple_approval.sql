...
with check (
  ...
  and exists (
    ...
    where projects.project_key = shared_project_members.project_key
      and shared_project_members.role = case when coalesce(projects.join_policy, 'open') = 'approval' then 'pending' else 'editor' end
  )
);
...
begin
  ...
  select
    ...
    coalesce(members.role, case when projects.owner_user_id = current_user_id then 'owner' else '' end),
    (projects.owner_user_id = current_user_id or coalesce(members.role, '') in ('owner', 'editor')),
    coalesce(projects.join_policy, 'open'),
    projects.updated_at,
    projects.created_at
  from public.shared_projects as projects
...
begin
  ...
  select
    ...
    coalesce(project_row.join_policy, 'open');
end;
$$;
...
begin
  ...
  select
    ...
    projects.title,
    case
      when coalesce(projects.join_policy, 'open') = 'open'
        or (
          current_user_id is not null
          and (projects.owner_user_id = current_user_id or coalesce(members.role, '') in ('owner', 'editor', 'viewer'))
        )
        then projects.latest_snapshot
      else null::jsonb
    end,
