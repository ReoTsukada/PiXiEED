...
declare
  ...
  normalized_session_id text := nullif(btrim(coalesce(target_session_id, '')), '');
...
begin
  ...
  if member_row.active_session_id is not null
     ...
     and member_row.active_session_id <> normalized_session_id then
  ...
  set
    ...
    active_device_id = coalesce(normalized_device_id, normalized_session_id),
