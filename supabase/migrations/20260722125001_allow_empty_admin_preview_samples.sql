-- 試聴を未選択にした商品は、サムネイルだけを再生成できるようにする。
-- 選択済みの試聴画像だけを保存するため、空配列も正当な入力として扱う。
...
begin
  ...
  values (auth.uid(), 'admin_listing_previews_rebuilt', 'market_asset', v_asset.id::text,
    jsonb_build_object('preview_object_path', input_preview_object_path, 'sample_count', cardinality(coalesce(input_sample_preview_paths, array[]::text[]))));
end; $$;
