-- 管理者による全件再生成も、960pxへ拡大した試聴画像そのものに固定サイズ透かしを焼き込む。
...
begin
  ...
  values (auth.uid(), 'admin_listing_previews_rebuilt', 'market_asset', v_asset.id::text,
    jsonb_build_object('preview_object_path', input_preview_object_path, 'sample_count', cardinality(coalesce(input_sample_preview_paths, array[]::text[])), 'watermark_version', 'baked-v4'));
end; $$;
