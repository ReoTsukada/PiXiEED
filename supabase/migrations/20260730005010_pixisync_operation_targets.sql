-- A pixel patch is only valid for its exact immutable raster target.
alter table collab_v1.operations
  add column canvas_id text not null default '' check (char_length(canvas_id) between 1 and 128),
  add column frame_id text not null default '' check (char_length(frame_id) between 1 and 128),
  add column layer_id text not null default '' check (char_length(layer_id) between 1 and 128),
  add column canvas_width integer not null default 1 check (canvas_width between 1 and 16384),
  add column canvas_height integer not null default 1 check (canvas_height between 1 and 16384),
  add constraint pixisync_operation_target_cell_count check (canvas_width::bigint * canvas_height::bigint <= 268435456);

revoke all on function public.pixisync_get_ops_since(uuid, bigint, integer) from public, anon, authenticated;
revoke all on function public.pixisync_commit_operation(uuid, uuid, uuid, text, bigint, smallint, bytea, bytea, integer, uuid) from public, anon, authenticated;
drop function public.pixisync_get_ops_since(uuid, bigint, integer);
drop function public.pixisync_commit_operation(uuid, uuid, uuid, text, bigint, smallint, bytea, bytea, integer, uuid);
...
returns table (revision bigint, operation_id uuid, actor_user_id uuid, actor_client_id uuid, kind text, structure_epoch bigint, codec_version smallint, canvas_id text, frame_id text, layer_id text, canvas_width integer, canvas_height integer, payload_b64 text, payload_sha256_hex text, pixel_count integer, undo_of_operation_id uuid, created_at timestamptz)
language plpgsql security definer set search_path = '' as $$
begin
  if not collab_v1.is_active_member(p_room_id) then raise exception 'not_active_member'; end if;
  return query select op.revision, op.operation_id, op.actor_user_id, op.actor_client_id, op.kind::text, op.structure_epoch, op.codec_version, op.canvas_id, op.frame_id, op.layer_id, op.canvas_width, op.canvas_height, encode(op.payload, 'base64'), encode(op.payload_sha256, 'hex'), op.pixel_count, op.undo_of_operation_id, op.created_at
