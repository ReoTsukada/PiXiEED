-- Canonical byte 0 is transparent; bytes 1..255 represent palette 0..254.
-- Replacing the validator before any PiXiSYNC operation exists keeps the
-- server and client codec contract identical.
create or replace function collab_v1.validate_pixel_patch(p_payload bytea, p_cell_count bigint)
returns integer
language plpgsql immutable set search_path = '' as $$
declare v_length integer := octet_length(p_payload); v_offset integer := 6; v_count bigint := 0; v_value bigint; v_shift integer; v_bytes integer; v_byte integer; v_index bigint := -1; v_delta bigint; v_i integer;
begin
  ...
  for v_i in 1..v_count loop
    ...
    v_offset := v_offset + 1; -- any byte is a valid canonical palette value.
  end loop;
